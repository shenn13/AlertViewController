...
typedef NS_ENUM(NSInteger,SSActionSheetType) {
    SSActionSheetTypeWithImg,
    SSActionSheetTypeWithTitle,
};
@interface SSActionSheet()

@property(nonatomic,assign) SSActionSheetType actionSheetType;
...
@implementation SSActionSheet

+(instancetype)ActionSheetWithTitleArray:(NSArray<NSString *> *)titleArray andTitleColorArray:(NSArray *)colors delegate:(id<SSActionSheetDelegate>)delegate{
...
-(instancetype)initSheetWithTitles:(NSArray *)titleArray andTitleColors:(NSArray *)colors andDelegate:(id<SSActionSheetDelegate>)delegate{
    self.titles = titleArray;
    self.colors = colors;
    self.actionSheetType = SSActionSheetTypeWithTitle;
    _delegate = delegate;
    return [self initActionSheet];
}
-(instancetype)initSheetWithImgs:(NSArray *)imgArray andDelegate:(id<SSActionSheetDelegate>)delegate{
    self.actionSheetType = SSActionSheetTypeWithImg;
...
-(instancetype)initActionSheet{
    if (self = [super init]) {
        ...
        switch (self.actionSheetType){
            case SSActionSheetTypeWithImg:
                arrayC = self.imgArray;
                break;
            case SSActionSheetTypeWithTitle:
                arrayC = self.titles;
                break;
            default:
                break;
        ...
        for (int i = 0; i<arrayC.count; ++i) {
            ...
            if (self.actionSheetType == SSActionSheetTypeWithTitle) {
                NSString *title = @"";
            ...
            }else if (self.actionSheetType == SSActionSheetTypeWithImg){
                NSString *imageName = @"";
...
-(void)buttonClick:(UIButton *)btn{
    ...
    [self.delegate SSActionSheetClickWithIndex:(int)btn.tag-990];
